...
// Barramento I2C 0 (Sensor de Cor)
#define I2C0_PORT i2c0
#define I2C0_SDA_PIN 0
#define I2C0_SCL_PIN 1

// Barramento I2C 1 (Display OLED)
...
// Guarda o estado atual do display (0 = RGB, 1 = Normalizado)
volatile int estado_display = 0;
// Usado para evitar múltiplos cliques de botão (debounce)
volatile uint32_t ultimo_tempo_clique = 0;

// =============================================================================
// --- Funções de Hardware e Interface ---
// =============================================================================

// Função chamada sempre que um botão é pressionado
void tratar_interrupcao_gpio(uint gpio, uint32_t events) {
    ...
    if (gpio == BOTAO_B_PIN) {
        estado_display = (estado_display + 1) % 2; // Avança para a próxima tela
    } else if (gpio == BOTAO_A_PIN) {
        estado_display = (estado_display - 1 + 2) % 2; // Volta para a tela anterior
    }
}

// Inicializa o pino do buzzer para usar PWM
...
// Toca um som com uma frequência (Hz) e duração (ms) específicas
void tocar_nota(uint frequencia, uint duracao_ms) {
    ...
    pwm_set_gpio_level(BUZZER_PIN, limite_wrap / 2); // 50% de ciclo de trabalho para o som
    sleep_ms(duracao_ms);
    pwm_set_gpio_level(BUZZER_PIN, 0); // Desliga o som
}

// Toca um alerta sonoro diferente para cada cor
...
// Desenha a tela com os valores RGB brutos
...
// Desenha a tela com os valores normalizados em percentagem
...
// Converte o nome de uma cor para o formato GRB da matriz, com brilho reduzido
...
int main() {
    ...
    while (1) {
        ...
        // 4. Desenha a tela correta no display
        ...
        // Pausa para evitar que o som toque sem parar
        sleep_ms(300); 
    }
}
