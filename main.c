...
void tocar_alerta(const char *nome_da_cor)
{
    ...
    else if (strcmp(nome_da_cor, "Verde") == 0)
    {
        tocar_nota(1200, 100);
        tocar_nota(1600, 150);
    }
    else if (strcmp(nome_da_cor, "Azul") == 0)
    {
        tocar_nota(800, 100);
        tocar_nota(1200, 100);
    }
    else if (strcmp(nome_da_cor, "Branco") == 0)
    {
        tocar_nota(1000, 80);
        tocar_nota(1200, 80);
        tocar_nota(1500, 80);
    }
    else if (strcmp(nome_da_cor, "Desconhecido") == 0)
    {
        tocar_nota(400, 500);
    }
}
...
// Desenha a tela com o valor de Lux do sensor BH1750
void desenhar_tela_lux(ssd1306_t *display, const char *str_lux)
{
    ssd1306_fill(display, false);
    ssd1306_draw_string(display, "Sensor de Luz", 15, 10, false);
    ssd1306_draw_string(display, str_lux, 30, 30, false); // Exibe o valor de lux
}
...
{
    // Calcula o divisor de brilho baseado no valor de lux (quanto mais claro, maior o divisor)
    // Escala: lux <= 50 -> divisor = 1 (máximo brilho)
    //        lux 51-300 -> divisor = 3 a 6
    //        lux 301-1000 -> divisor = 7 a 9
    //        lux > 1000 -> divisor = 10 (mínimo brilho)
    int DIVISOR_DE_BRILHO;
    if (lux <= 50)
        DIVISOR_DE_BRILHO = 1;
    else if (lux <= 300)
        DIVISOR_DE_BRILHO = 3 + ((lux - 51) * 3) / 249; // 3 a 6
    else if (lux <= 1000)
        DIVISOR_DE_BRILHO = 7 + ((lux - 301) * 2) / 699; // 7 a 9
    else
        DIVISOR_DE_BRILHO = 10;

    printf("Lux: %d, Divisor de Brilho: %d\n", lux, DIVISOR_DE_BRILHO);
...
int main()
{
    ...
    while (1)
    {
        ...
        case 1:
            ...
            break;
        case 2:
            desenhar_tela_lux(&display, str_lux);
            break;
        }
        ssd1306_send_data(&display);
